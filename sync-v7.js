...
(() => {
  ...
  const canonicalItemName = name => String(name || '').trim().normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLocaleLowerCase('pt-BR');
  const mergeState = (base, incoming) => {
    ...
    ].map(canonicalItemName).filter(Boolean));
    ...
    for (const collection of ['cartoes', 'contasFixas', 'contasAdicionais']) {
      for (const item of b[collection] || []) deleted.delete(canonicalItemName(item.nome));
    }
    const mergeItems = (left = [], right = []) => {
      ...
      const addItem = item => {
        ...
        const current = { ...item, nome: name, meses: { ...(item.meses || {}) } };
        ...
        if (index === undefined) {
          ...
          result[index] = {
            ...result[index],
            ...current,
            nome: result[index].nome || current.nome,
            meses: { ...(result[index].meses || {}), ...(current.meses || {}) }
          };
        }
      };
      for (const item of left || []) addItem(item);
      for (const item of right || []) addItem(item);
      return result;
    };
    return {
      ...a,
  ...
  const normalizeFlatState = state => {
    ...
    for (const collection of ['cartoes', 'contasFixas', 'contasAdicionais']) {
      ...
      for (const item of (result[collection] || [])) {
        const name = String(item.nome || '').trim();
        const key = canonicalItemName(name);
        if (!key || result.deletedItems.some(deleted => canonicalItemName(deleted) === key)) continue;
        const normalized = {
          ...item,
          nome: name,
        ...
        if (previous) {
          previous.meses = { ...(previous.meses || {}), ...(normalized.meses || {}) };
          previous.total = Object.values(previous.meses).reduce((sum, value) => sum + normalizeMoney(value), 0);
          continue;
        }
        normalized.total = Object.values(normalized.meses).reduce((sum, value) => sum + normalizeMoney(value), 0);
        seenItems.set(key, normalized);
        uniqueItems.push(normalized);
      }
      result[collection] = uniqueItems;
    }
  ...
  async function writeRemote() {
    ...
    try {
      ...
      if (parsed?.resetAll) {
        ...
        nextData = normalizeStoredState(parsed);
      } else {
        const local = normalizeStoredState(parsed);
        const { data: existing, error: readError } = await client.from(TABLE).select('data').eq('user_id', user.id).maybeSingle();
        if (readError) throw readError;
        nextData = mergeStoredState(existing?.data || {}, local);
      }
      ...
      console.error('[CONTROLE DE GASTOS] Falha ao salvar dados na nuvem:', error);
      message('Os dados foram salvos neste aparelho, mas não foi possível sincronizar com a nuvem. Verifique sua conexão e tente novamente.');
    } finally {
      saving = false;
      if (pendingRemoteWrite) {
        clearTimeout(saveTimer);
        saveTimer = setTimeout(() => void writeRemote(), 100);
      }
    }
  }
  ...
  async function syncCurrentUser() {
    ...
    let nextData;
    if (EMPTY_TEST || parsedLocal?.resetAll) {
      nextData = {};
    } else if (data?.data) {
      nextData = mergeStoredState(data.data, parsedLocal);
    } else {
      nextData = normalizeStoredState(parsedLocal);
    }
    applyingRemote = true;
    localStorage.setItem(DATA_KEY, JSON.stringify(nextData));
    applyingRemote = false;
    ...
    if (writeError) throw writeError;
    client.channel(`dashboard-${user.id}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: TABLE, filter: `user_id=eq.${user.id}` }, (payload) => {
        if (!payload.new?.data || payload.new.user_id !== user.id) return;
        try {
          applyingRemote = true;
          ...
          applyingRemote = false;
          console.error('[CONTROLE DE GASTOS] Falha ao receber atualização remota:', eventError);
        }
      })
      .subscribe((status) => {
        if (status === 'CHANNEL_ERROR' || status === 'TIMED_OUT') {
          console.warn('[CONTROLE DE GASTOS] Canal de sincronização indisponível:', status);
        }
      });
  }
  ...
  async function start() {
    try {
      ...
      try {
        ...
        console.error('[CONTROLE DE GASTOS] Sessão restaurada, mas a sincronização falhou:', syncError);
      }
      showApp();
    } catch (error) {
  ...
  document.getElementById("cloud-login-form").addEventListener("submit", async (event) => {
    ...
    try {
      ...
      try {
        ...
        console.error('[CONTROLE DE GASTOS] Autenticação concluída, mas a sincronização falhou:', syncError);
      }
      showApp();
    } catch (error) {
