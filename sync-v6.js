...
(() => {
  ...
  const normalizeItemName = name => String(name || '').trim().normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLocaleLowerCase('pt-BR');
  const mergeState = (base, incoming) => {
    ...
    for (const collection of ['cartoes', 'contasFixas', 'contasAdicionais']) {
      for (const item of b[collection] || []) deleted.delete(normalizeItemName(item.nome));
    }
  ...
  const normalizeFlatState = state => {
    ...
    for (const collection of ['cartoes', 'contasFixas', 'contasAdicionais']) {
      ...
      for (const item of (result[collection] || [])) {
        const name = String(item.nome || '').trim();
        const key = normalizeItemName(name);
        if (!key || result.deletedItems.some(deleted => normalizeItemName(deleted) === key)) continue;
        const normalized = {
          ...item,
          nome: name,
